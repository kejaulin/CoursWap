...
function EspaceProf({ nom, disponibilites, onEdit, onRetourAccueil, meetings, user }) {
  return (
          ...
          <ul className="space-y-4">
            {meetings.map(meeting => (
              <li className="border border-purple-300 rounded-xl p-4 shadow hover:shadow-lg transition-shadow"
               key={meeting._id} style={{ marginBottom: '1rem' }}>
                <div className="flex justify-between items-center mb-2">
              <span className="text-sm text-gray-600">
                <strong>Date :</strong> {moment(meeting.date).format("DD/MM/YYYY")}
              </span>
              <span className="text-sm text-gray-600">
                <strong>Heure :</strong> {meeting.heure}
              </span>
            </div>
            ...
            <div className="flex gap-6 text-gray-800 font-medium">
              <div>
                ...
                {String(meeting.profId._id) === String(user._id) ? "Vous" : meeting.profId.nom}
              </div>
              <div>
                <strong>Étudiant :</strong>{" "}
                {String(meeting.etudiantId._id) === String(user._id) ? "Vous" : meeting.studentId.nom}
              </div>
            </div> </li>
            ))}
          </ul>
